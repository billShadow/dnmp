    ...
    public static function weachatPostPemCurl($url, $vars, $type=0)
    {
        if ($type) {
            $rs = Curl::to( $url )
                ...
                ->withOption('SSLCERT',app_path('/lib/WeChat/Transfer/cert/king_apiclient_cert.pem'))
                ->withOption('SSLKEY',app_path('/lib/WeChat/Transfer/cert/king_apiclient_key.pem'))
                ->withData( $vars )
                ->post();
            return $rs;
        } else {
...
//    }
    ...
    public static function make_char( $length = 6 ){
        $chars = array('a', 'b','5','6', 'e', 'f', 'g', 'h',
            'i', 'j',  'n', '1','2','o', 'p', 'q', 'r', 's',
            't', 'u', 'v', '4','7','w', 'x', 'z','0','3','y','8','9', 'c', 'd');
        $str = '';
        for($i = 0; $i < $length; $i++){
    ...
    public static function getDir( $base )
    {
     ...
     */
    static public function getImgPath($file , $dir = '')
    {
        if(isset($file) && !empty($file))
        {
            if(!$file['error'] && $file['size'] > 0) // 判断是否有文件
            {
                $path = public_path(env('UPLODE_PATH', '/web/uplode/')); // 获取图片目录路径
                $avatar = $file['tmp_name']; // 获取图片名
                $hash = md5($avatar);        // 图片名加密
                $newloc = Util::getDir($path, $hash).'/'.$hash.'.jpg';
                if ( move_uploaded_file($avatar, $newloc) )//创建图片
                {
                    $row = [];
                    $row['path'] = base64_encode(date('Y-m-d').'/'.$hash);
                    return $row;
                }
                else
                {
     ...
     * @return multitype:unknown |string
     */
    static public function getGifImgPath($file)
    {
        if(isset($file) && !empty($file))
        {
            if(!$file['error'] && $file['size'] > 0) // 判断是否有文件
            {
                ...
                $type = !empty($type)?$type:( !empty(substr(strrchr($file['type'], '/'), 1)) ? substr(strrchr($file['type'], '/'), 1) :'jpg' );
                $newloc = Util::getDir($path, $hash).'/'.$hash.'.'.$type;
                if ( move_uploaded_file($avatar, $newloc) )//创建图片
                {
                    $row = [];
                    $row['path'] = base64_encode(date('Y-m-d').'/'.$hash.'.'.$type);
                    return $row;
                }
                else
                {
     ...
     * 获取图片
     */
    static public function getImg($path)
    {
        $url = '';
        if( !empty($path) )
        {
            $http = substr($path, 0, strpos($path, ':'));
            if (in_array($http, ['http', 'https'])) {
                return $path;
            }
            $url = env('UPLODE_URL','http://www.osv-cloud.dev/web/uplode');
            $url = $url.'/'.base64_decode($path).'.jpg';
        }
        return $url;
    }
    static public function getImgReset($path)
    {
        $url = '';
        if( !empty($path) )
        {
     ...
     * 获取图片
     */
    static public function getGifImg($path)
    {
        $url = '';
        if( !empty($path) )
        {
